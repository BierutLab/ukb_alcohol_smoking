---
title: "Regression with scaled predictors and total volumes"

---

7/25/24 Predictors and all IDPs scaled (z-score normalized, (x-mean)/sd) in results from this model
...
all_IDP_results <- all_IDP_results %>%
  dplyr::rename("scaled_interaction_beta" = `scaled_week_drinks:scaled_pack_years_beta`) %>%
  dplyr::rename("scaled_interaction_se" = `scaled_week_drinks:scaled_pack_years_se`) %>%
  dplyr::rename("scaled_interaction_p" = `scaled_week_drinks:scaled_pack_years_p`)
...
save(all_IDP_results, file = paste0(output_path, "regression_scaled_", Sys.Date(), ".RData"))
