...
9/23/24 Files needed to run this script are not UKB confidential data and are included in the repo
I have also included the output of this script in the repo
...
dMRI: for dMRI IDPs only, which DTI or NODDI measure the IDP represents
...
9/23/24 This file is included in the repo, put it in whatever you are using as input path
...
```{r}
# Load IDP names
IDP_names <- read.delim(file = paste0(input_path, "IDP_name_list.txt"), sep = "#", header = FALSE)
...
There are 99 IDP listed on the showcase, as of 9/8/21 I am getting one more than this
There must be one in between the bounds that is not in the showcase?
...
Why am I getting one more than the showcase lists for the last couple?
...
region_mapping <- read.csv(paste0(input_path, "Yoonhoo_assigned_IDP_regions_11-30-21.csv"))
