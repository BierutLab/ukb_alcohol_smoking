...
Plot of the interaction in total brain volume, to contrast with T2* interaction
...
Beta = 0.019, p = 6.1e-7
Interaction in total volume does not reach 0.01 threshold
```{r}

total_volume <- all_IDP_results %>%
  filter(numeric_IDP == 26514)

```


Make plots of total volume by drinks per week, stratified on pack years
```{r}
...
total_volume_pred <- data.frame("dpw" = dpw, "pack_years" = pack_years)

```

8/4/24 referencing regression with scaled predictors and scaled total volume measure
...
total_volume_pred <- total_volume_pred %>%
  mutate(dpw_direct = dpw_z * total_volume$scaled_week_drinks_beta) %>%
  mutate(pack_years_direct = pack_years_z * total_volume$scaled_pack_years_beta)
...
total_volume_pred <- total_volume_pred %>%
  mutate(pred = dpw_direct + pack_years_direct +
           total_volume$scaled_interaction_beta * dpw_z * pack_years_z)
...
total_volume <- total_volume %>%
  mutate(dpw_beta_lower = scaled_week_drinks_beta - 1.96 * total_volume$scaled_week_drinks_se) %>%
  mutate(dpw_beta_upper = scaled_week_drinks_beta + 1.96 * total_volume$scaled_week_drinks_se) %>%
  mutate(pack_years_beta_lower = scaled_pack_years_beta - 1.96 * total_volume$scaled_pack_years_se) %>%
  mutate(pack_years_beta_upper = scaled_pack_years_beta + 1.96 * total_volume$scaled_pack_years_se) %>%
  mutate(interaction_beta_lower = scaled_interaction_beta - 1.96 * total_volume$scaled_interaction_se) %>%
  mutate(interaction_beta_upper = scaled_interaction_beta + 1.96 * total_volume$scaled_interaction_se)
...
total_volume_pred <- total_volume_pred %>%
  mutate(dpw_direct_lower = dpw_z * total_volume$dpw_beta_lower) %>%
  mutate(dpw_direct_upper = dpw_z * total_volume$dpw_beta_upper) %>%
  mutate(pack_years_lower = pack_years_z * total_volume$pack_years_beta_lower) %>%
  mutate(pack_years_upper = pack_years_z * total_volume$pack_years_beta_upper) %>%
mutate(pred_lower = dpw_direct_lower + pack_years_lower +
           total_volume$interaction_beta_lower * dpw_z * pack_years_z) %>%
  mutate(pred_upper = dpw_direct_upper + pack_years_upper +
           total_volume$interaction_beta_upper * dpw_z * pack_years_z)
...
pack_years_plot <- total_volume_pred %>%
  ...
  ggtitle("Predicted decrease in total volume") +
  xlab("Pack years") +
  ylab("Total volume (mm3, normalized)") +
  scale_x_discrete(expand = c(0.01, 0.01)) +
  scale_color_viridis(discrete = TRUE, end = 0.8) +
  scale_fill_viridis(discrete = TRUE, end = 0.8)

ggsave(
  "pack_years_interaction_tv.png",
  pack_years_plot,
  width = 4,
  height = 3,
  dpi = set_dpi)
...
ggsave(
  "week_drinks_interaction_tv.png",
  dpw_plot,
  width = 4,
  height = 3,
  dpi = set_dpi)
